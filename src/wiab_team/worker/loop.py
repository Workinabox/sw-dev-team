...
async def work(
    ...
    *,
    ...
    while not stop.is_set():
        ...
        await _run_one(board, task, settings, config, checkpointer=checkpointer, run=run)
        completed += 1
...
async def _run_one(
    ...
    *,
    ...
    Every exit from this function reports *something* to the backend. A task
    left `in_progress` because the team crashed is invisible: no other team can
    take it and no human is told it stalled.
    """
    log.info("task_claimed", task_id=task.task_id, work_id=task.work_id, title=task.title)
    try:
    ...
    try:
        ...
        result: TeamRunResult = await run(payload, config=config, checkpointer=checkpointer)
